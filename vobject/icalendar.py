        ...
        for transitionTo in 'daylight', 'standard':
            for rule in completed[transitionTo]:
                ...
                if rule['plus'] is not None:
                    ...
                    num = None
                if num is not None:
                    dayString = ";BYDAY=" + six.u(num) + WEEKDAYS[rule['weekday']]
                else:
                    dayString = ""
                ...
                for rule in setlist:
                    ...
                    if rule._bymonth is not None and len(rule._bymonth) > 0:
                        if (rule._byweekday is not None or
                            ...
                            values['BYMONTH'] = [six.u(n) for n in rule._bymonth]

                    if rule._byyearday is not None:
                        values['BYYEARDAY'] = [six.u(n) for n in rule._byyearday]
                    if rule._byweekno is not None:
                        values['BYWEEKNO'] = [six.u(n) for n in rule._byweekno]
